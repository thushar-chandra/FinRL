"""Risk Assessment RL Agent.

Learns to estimate per-asset volatility and risk score from engineered
features (including regime features), trained via Stable-Baselines3 PPO
within the FinRL ecosystem.

Reference: docs/architecture/AGENTS.md §2,
docs/architecture/INTERFACE_CONTRACTS.md §2,
docs/architecture/MODULE_SPECIFICATIONS.md §2.
"""
...
# ---------------------------------------------------------------------------
# Regime-column detection  (shared heuristic with market_agent.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Internal Gymnasium environment for risk-assessment RL training
# ---------------------------------------------------------------------------

_VOL_NORMALIZATION_FACTOR = 10.0


class _RiskAssessmentEnv(gym.Env):
    """Gymnasium environment for training the Risk Assessment RL agent.

    **Observation** — engineered feature vector (including regime features)
    at the current timestep.

    **Action** — per-asset continuous values:
        ``action[i]``                     → expected_volatility for asset *i*
        ``action[n_assets + i]``          → risk_score for asset *i*
    Both are in ``[0, 1]``.

    **Reward** — negative mean absolute error between predicted and realized
    values.  Realized volatility is proxied by the absolute forward return
    (scaled and clipped to ``[0, 1]``).  Realized risk is the same proxy
    (the two outputs share a reward signal at training time so the agent
    learns both from a single accuracy target).
    """
    ...
    def __init__(
        ...
        self.action_space = spaces.Box(
            low=0.0, high=1.0, shape=(2 * n_assets,), dtype=np.float32,
        )
    ...
    def _compute_reward(self, action: np.ndarray) -> float:
        ...
        realized_vol = np.clip(
            np.abs(forward_ret) * _VOL_NORMALIZATION_FACTOR, 0.0, 1.0,
        )
        realized_risk = realized_vol.copy()

        pred_vol = action[:self._n_assets]
        pred_risk = action[self._n_assets:]

        vol_error = np.abs(pred_vol - realized_vol)
        risk_error = np.abs(pred_risk - realized_risk)

        mean_error = float(np.mean(vol_error + risk_error) / 2.0)
        return -mean_error
...
class RiskAssessmentAgent:
    """Risk / volatility RL agent trained via PPO.

    Produces a per-asset ``expected_volatility`` and ``risk_score`` from
    engineered features (including regime features).  Feeds ``AgentOutput``
    to Confidence Estimation & Calibration and to Confidence-Aware Decision
    ...
    def train(
        ...
        env = _RiskAssessmentEnv(feature_array, ret_array, n_assets)
    ...
    # ------------------------------------------------------------------
    # Public inference interface  (INTERFACE_CONTRACTS.md §2)
    # ------------------------------------------------------------------

    def predict(self, features: pd.DataFrame) -> AgentOutput:
        """Produce an expected-volatility and risk-score estimate per asset.
        ...
        Returns:
            ...
            each ticker in ``universe`` to
            ``{"expected_volatility": float, "risk_score": float}``.
        ...
        for i, ticker in enumerate(self._universe):
            recommendation[ticker] = {
                "expected_volatility": float(action[i]),
                "risk_score": float(action[n_assets + i]),
            }
    ...
    def _build_reasoning(
        ...
        avg_vol = float(np.mean([v["expected_volatility"] for v in recommendation.values()]))
        avg_risk = float(np.mean([v["risk_score"] for v in recommendation.values()]))
        return (
            f"RiskAssessmentAgent: avg_volatility={avg_vol:.4f}, "
            f"avg_risk_score={avg_risk:.4f} "
